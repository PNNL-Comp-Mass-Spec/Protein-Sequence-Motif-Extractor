...
#End Region
    ...
    ''' <remarks></remarks>
    Protected Sub FindMatchingMotifs(ByRef swMotifsOutputFile As System.IO.StreamWriter, _
                                     ByVal blnOutputAsDelimitedText As Boolean, _
                                     ByRef udtProtein As udtProteinInfoType, _
                                     ByVal strMotif As String, _
                                     ByRef reMotif As System.Text.RegularExpressions.Regex)

        Static sbResidues As New System.Text.StringBuilder
            ...
            Do
                ...
                If intMatchIndex >= 0 Then
                    ...
                    If blnOutputAsDelimitedText Then
                        ' Tab-delimited text
                        swMotifsOutputFile.WriteLine(udtProtein.Name & ControlChars.Tab & _
                                                     strMatchResidues & ControlChars.Tab & _
                                                     intResidueLoc & ControlChars.Tab & _
                                                     sbResidues.ToString)
                    Else
                        ' Fasta format
                        swMotifsOutputFile.WriteLine(FastaFileOptions.ProteinLineStartChar & udtProtein.Name & _
    ...
    Protected Function OpenInputFile(ByVal strInputFilePath As String, _
                                     ...
                                     ByRef objProteinFileReader As ProteinFileReader.ProteinFileReaderBaseClass, _
                                     ByRef blnUseUniqueIDValuesFromInputFile As Boolean, _
        ...
        Try
            ...
            Else
                ...
                If strOutputFileName.Length = 0 Then
                    ...
                    If mOutputMotifsAsDelimitedTextFile Then
                        strOutputFileName = System.IO.Path.GetFileNameWithoutExtension(strInputFilePath) & "_Motifs.txt"
                    Else
                        strOutputFileName = System.IO.Path.GetFileNameWithoutExtension(strInputFilePath) & "_Motifs.fasta"
                    End If
                End If

                ' Make sure the output file isn't the same as the input file
                If System.IO.Path.GetFileName(strInputFilePath).ToLower = System.IO.Path.GetFileName(strOutputFileName).ToLower Then
                ...
                strOutputMotifsFilePath = System.IO.Path.Combine(strOutputFolderPath, strOutputFileName)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function ParseProteinFile(ByVal strProteinInputFilePath As String, _
                                     ByVal strOutputFolderPath As String, _
        ...
        Try
            ...
            blnSuccess = OpenInputFile(strProteinInputFilePath, _
                            strOutputFolderPath, _
                            strOutputFileNameBaseOverride, _
                            objProteinFileReader, _
                            blnUseUniqueIDValuesFromInputFile, _
            ...
            Try
                ...
                swMotifsOutputFile = New System.IO.StreamWriter(New System.IO.FileStream(strOutputMotifsFilePath, IO.FileMode.Create, IO.FileAccess.Write, IO.FileShare.Read))
                blnSuccess = True
            Catch ex As Exception
